...
from NTRU.utility_mod2 import BASE, _P, max_V_coeffi, bytes_per_coeffi

V_space = (max_V_coeffi * bytes_per_coeffi) // 4
round_half = _P // BASE

def f_proto():
    print("extern int jump%ddivsteps_mod2(int minusdelta, uint32_t *M1, uint32_t *f, uint32_t *g);" % (BASE))
    print("int jump%ddivsteps_mod2_%d(int minusdelta, uint32_t *M, uint32_t *f, uint32_t *g);\n" % (_P*2, BASE))
    print("extern void __update_fg_%dx%d_mod2(uint32_t *f, uint32_t *g, uint32_t *M1);" % (BASE, _P))
    for i in range(1, round_half):
        print("extern void __update_VS_%dx%d_mod2(uint32_t *V, uint32_t *S, uint32_t *M1);" % (BASE, BASE*i))
    
    #2
    if BASE * round_half != _P:
        print("extern void __update_VS_%dx%d_mod2(uint32_t *V, uint32_t *S, uint32_t *M1);" % (BASE, BASE*round_half))
    ...
    # Phase 3:
    print("extern void __update_VS_%dx%d_mod2(uint32_t *V, uint32_t *S, uint32_t *M1);" % (BASE, _P))
    if _N_max % BASE == 0:
            print("extern void __update_VS_%dx%d_mod2(uint32_t *V, uint32_t *S, uint32_t *M1);" % (BASE, max_V_coeffi))
    for i in range(round_half_2):
        print("extern void __update_fg_%dx%d_mod2(uint32_t *f, uint32_t *g, uint32_t *M1);" % (BASE, _N_max - BASE*i))
    ...
    while(_N_max_2 != 0):
        print("extern int jump%ddivsteps_mod2(int minusdelta, uint32_t *M1, uint32_t *f, uint32_t *g);" % (base))
        print("extern void __update_fg_%dx%d_mod2(uint32_t *f, uint32_t *g, uint32_t *M1);" % (base, _N_max_2))
        if _N_max_2 - base == 0:
            print("extern void __update_VS_%dx%d_mod2(uint32_t *V, uint32_t *S, uint32_t *M1);" % (base, max_V_coeffi))
        else:
            print("extern void __update_VS_%dx%d_mod2(uint32_t *V, uint32_t *S, uint32_t *M1);" % (base, _P))
...
for i in range(round_half):
    ...
    if i == 0:
        ...
        printIn("__update_VS_%dx%d_mod2(V, S, M1+%d);" % (BASE, BASE*i, uvrs_pos))
...
for i in range(round_half_2):
    ...
    if i == round_half_2 - 1 and _N_max % BASE == 0:
        ...
        printIn("__update_VS_%dx%d_mod2(V, S, M1+%d);" % (BASE, _P, uvrs_pos))
...
while(_N_max_2 != 0):
    ...
    if _N_max_2 - base == 0:
        ...
        printIn("__update_VS_%dx%d_mod2(V, S, M1+%d);" % (base, _P, _uvrs_pos))
