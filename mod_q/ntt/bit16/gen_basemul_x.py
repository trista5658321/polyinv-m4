...
def poly_4x4(tmp):
    poly_4 = ["r"+str(x) for x in range(1,6)]
    get_w()
    printIn("ldrsh.w r3, [r0, #2]")
    printIn("ldrsh.w r4, [r0, #4]")
    printIn("ldrsh.w r5, [r0, #6]")
    printIn("ldrsh.w r2, [r0]")

    # 3 -> 1
    printIn("smulbb %s, %s, %s" % (poly_4[0], r_w, poly_4[4]))
    combine_two_coeffi(poly_4[0], poly_4[0], poly_4[1], tmp)
    combine_two_coeffi(poly_4[1], poly_4[2], poly_4[3], tmp)
    return poly_4
