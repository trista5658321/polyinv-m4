...
void gf_polymul_128x256_2x2_x_2x2_onlyuv (int *M, int *M1, int *M2){ // M = M2*M1, length M1 : 512*4   M2 : 32*4
  ...
  gf_polymul_128x256(M+192, M2+64, M1+384); // v2 * s1
  for (i=192, Y=B753_1; i>0; i--) {	// v = x u2 v1 + v2 s1
    T = barrett_16x2i(__SADD16(*X,*(Y++)));
    *(X++) = T;
  }
}

void gf_polymul_384x512_2x2_x_2x2_onlyuv (int *M, int *M1, int *M2){ // M = M2*M1, length M1 : 512*4   M2 : 32*4
  int i, T, *X, *Y;
  ...
  B753_1[0] = 0;
  gf_polymul_384x512(BB753_1, M2, M1); // x * u2 * u1 
  gf_polymul_384x512(M, M2+192, M1+512); // v2 * r1
  for (i=448, X=M, Y=B753_1; i>0; i--) {	// u = x u2 u1 + v2 r1
    T = barrett_16x2i(__SADD16(*X,*(Y++)));
    *(X++) = T;
  }

  gf_polymul_384x512(BB753_1, M2, M1+256); // x * u2 * v1
  gf_polymul_384x512(M+477, M2+192, M1+768); // v2 * s1
  for (i=448, X=M+477, Y=B753_1; i>0; i--) {	// v = x u2 v1 + v2 s1
    T = barrett_16x2i(__SADD16(*X,*(Y++)));
    *(X++) = T;
  }
}

// M: f(128), g(128), u(954), v(954)
int jump896divsteps(int minusdelta, int *M, int *f, int *g){
  int i;
  int M1[1536], M2[768], M3[384], fg1[384], fg2[128];
  ...
  gf_polymul_128x256_2x2_x_2x2_onlyuv(fg1,M2+256,M3+128);
  gf_polymul_384x512_2x2_x_2x2_onlyuv(M+128,M1+512,fg1);
